...
int creaHvacio(char *fichHash,regConfig *reg){
   ...
   if(fwrite(reg,sizeof(regConfig),1,f)==0 && ferror(f))
   {
      return -5;
   }
   for ( i = 0; i < totC; i++)
   {
      if(fwrite(&c,sizeof(tipoCubo),1,f)==0 && ferror(f)){
         return -5;
      }
   }
   if(fclose(f)==0){
...
int busquedaHash(FILE *fHash, tipoReg *reg, tPosicion *posicion){
   int numC=funcionHash(reg,config.nCubos);
   int i=0;
   //REcorrido de Registros
   for (i = 0; i < cubo.numRegAsignados; i++)
   {
      if (cmpClave(&cubo.reg[i],reg)==1)
      {
...
int modificarReg(FILE *fHash, tipoReg *reg, tPosicion *posicion){
   tipoReg new=*reg;
   int ret;
   ret=busquedaHash(fHash,&new,posicion);
